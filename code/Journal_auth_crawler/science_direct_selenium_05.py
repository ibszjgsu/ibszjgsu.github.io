...
def collect_paper_info(urls_in_a_journal, jour_name):
    ...
    for i in range(401,paper_url_in_a_journal.shape[0]):
    #for j, row in paper_url_in_a_journal.iterrows():

        # try if can be done by requests
        try:
            res = HtmlDownloader().download_request(paper_url_in_a_journal.loc[i,'paper_url'])
        except:
            print('stop at step: %s' % i)
        soup = BeautifulSoup(res, 'html.parser')
        tmp_info = {}
        tmp_info['url'] = paper_url_in_a_journal.loc[i,'paper_url']
        ...
        try:
            ...
            if not tmp_info['au_email']:
                print('Has to use Webdriver: %s' % paper_url_in_a_journal.loc[i,'paper_url'])
                try:
                    driver = HtmlDownloader().download_chrome_driver(paper_url_in_a_journal.loc[i,'paper_url'])
            ...
            print('The paper can not be retrievaled: %s' % paper_url_in_a_journal.loc[i,'paper_url'])
        if np.mod(i,10) == 0 and i > 0 or paper_url_in_a_journal.shape[0] - i < 10:
             # save into database
            param = {'table': jour_name, 'db': 'science_direct', 'if_exists':'append'}
