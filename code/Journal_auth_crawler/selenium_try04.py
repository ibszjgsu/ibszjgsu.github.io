...
def collect_from_paper2(paper_url):
    ...
    try:
        journal_info['url'] = paper_url
        journal_info['article_title'] = soup.find(class_='title-text').text
        ...
        # email address
        email_pat = '"href":"mailto:(.*?)"'
        emails = re.findall(email_pat, res.text)  # email
        journal_info['emails'] = ', '.join(emails)
        try:
            journal_info['abstract'] = ' '.join([it.text for it in soup.find(class_ = 'abstract author').find_all('p')])
        except:
            journal_info['abstract'] = 'Null'
        try:
            tmp = [it.find_all(class_='keyword')  for it in soup.find_all(class_='keywords-section') if it.find(class_='section-title').text == 'Keywords'][0]
