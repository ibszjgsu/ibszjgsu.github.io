...
def get_an_valid_requests(target_url):
...
#        'Referer': 'http: // www.xicidaili.com/wn',
    ...
    try:
        res = requests.get(target_url, timeout = 3, headers = my_headers)
        res.encoding = 'utf-8'
        soup = BeautifulSoup(res.text, 'html.parser')
        if keys in soup.find('title').text.split('.')[0]:
            return res  
...
#    res.encoding = 'utf-8'
    res = get_an_valid_requests(journal_url)
    soup = BeautifulSoup(res.text, 'html.parser')
...
#    res_issue.encoding = 'utf-8'
    res_issue = get_an_valid_requests(jou_all_issue_url)
...
#        res_issue.encoding = 'utf-8'
        res_issue = get_an_valid_requests(jou_all_issue_url_new)
        soup = BeautifulSoup(res_issue.text, 'html.parser')
...
#    res.encoding = 'utf-8'
    res = get_an_valid_requests(issue_url)
    soup = BeautifulSoup(res.text, 'html.parser')
...
#    res.encoding = 'utf-8'
    try:
        res = get_an_valid_requests(paper_url)
        soup = BeautifulSoup(res.text, 'html.parser')   
        journal_info = {}
