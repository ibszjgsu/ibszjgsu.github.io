...
if not table_exist('jrsssb'):
    #build a new table named by the journal title 
    sql = "create table jrsssb (id int not null unique auto_increment, \
...
"""
pagenum = 453
pageurl ='http://apps.webofknowledge.com/summary.do?product=WOS&colName=WOS&qid=4&SID=8CSVaV6ZzmFfzIvxbO6&search_mode=GeneralSearch&formValue(summary_mode)=GeneralSearch&update_back2search_link_param=yes&page='



for pg in range(183,pagenum + 1):
...
#    # Find the list of Date
    ...
    for i in range(0,len(paper_suburls)):
        ...
        sql_ins = "insert into jrsssb (title, authors, au_email, citation, volume, issue, year, url) \
