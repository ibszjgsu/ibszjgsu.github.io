...
if not table_exist('ejos'):
    #build a new table named by the journal title 
    sql = "create table ejos (id int not null unique auto_increment, \
...
pagenum = 92
'''
The following code is only valid for EJoS 
'''
# This is the prefix of page 2, 3,..., pagenum

pageurl ='http://apps.webofknowledge.com/summary.do?product=UA&colName=&qid=2&SID=7D8APErx3JDsmorzdcs&search_mode=GeneralSearch&formValue(summary_mode)=GeneralSearch&update_back2search_link_param=yes&page='
...
#    # Find the list of Date
    ...
    for i in range(1,len(paper_suburls)):
        ...
        sql_ins = "insert into ejos (title, authors, au_email, citation, volume, issue, year, url) \
