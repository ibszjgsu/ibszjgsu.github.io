...
if not table_exist('jsas'):
    #build a new table named by the journal title 
    sql = "create table jsas (id int not null unique auto_increment, \
         title varchar(300), authors varchar(300), au_email varchar(300),\
         citation varchar(20), url varchar(300),\
         primary key(id))"
    cur.execute(sql)
...
firsturl = 'http://apps.webofknowledge.com/summary.do?product=UA&colName=&qid=2&SID=8AdkR7zG2CwWT9yxRWs&search_mode=GeneralSearch&formValue(summary_mode)=GeneralSearch&update_back2search_link_param=yes&page=1'
...
'''
The following code is only valid for JSAS from Vol 189 issue 2 to Vol   issue  
'''
# This is the prefix of page 2, 3,..., pagenum

pageurl ='http://apps.webofknowledge.com/summary.do?product=UA&colName=&qid=2&SID=8AdkR7zG2CwWT9yxRWs&search_mode=GeneralSearch&formValue(summary_mode)=GeneralSearch&update_back2search_link_param=yes&page='

for pg in range(52,pagenum + 1):
...
#    paper_vols_pat = 'Volume: </span> <value>(.*?)</value>'
#    paper_vols = re.findall(paper_vols_pat,subres.text, re.S)
...
#    paper_dates = re.findall(paper_dates_pat,res.text, re.S)
    ...
    for i in range(0,len(paper_suburls)):
        ...
        paper_cites = re.findall(paper_cites_pat,subres.text, re.S)    
        ...
        sql_ins = "insert into jsas (title, authors, au_email, citation, url) \
        values ('%s', '%s', '%s', '%s', '%s')" %\
        (paper_titles[i].replace("'","").replace('"',''), paper_auths[i].replace("'","").replace('"',''), paper_email, paper_cites[0], paper_suburl)
#        try:
#            cur.execute(sql_ins)
#            conn.commit()
#        except:
