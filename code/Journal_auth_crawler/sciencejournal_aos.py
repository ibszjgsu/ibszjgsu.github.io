...
pageurl ='http://apps.webofknowledge.com/summary.do?product=UA&colName=&qid=1415&SID=8AdkR7zG2CwWT9yxRWs&search_mode=GeneralSearch&formValue(summary_mode)=GeneralSearch&update_back2search_link_param=yes&page='
...
#    paper_dates = re.findall(paper_dates_pat,res.text, re.S)
    ...
    for i in range(1,len(paper_suburls)):
        ...
        sql_ins = "insert into aos (title, authors, au_email, citation, url) \
        values ('%s', '%s', '%s', '%s', '%s')" %\
        (paper_titles[i].replace("'","").replace('"',''), paper_auths[i].replace("'","").replace('"',''), paper_email, cites, paper_suburl)
#        try:
#            cur.execute(sql_ins)
#            conn.commit()
#        except:
