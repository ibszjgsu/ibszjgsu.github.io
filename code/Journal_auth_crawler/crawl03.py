...
class DataSaver(object):
    ...
    def __init__(self):
        ...
        self.pw = "11031103"
        self.charset = "utf8"
        self.sql_eng = 'mysql+mysqldb://root:11031103@10.23.0.2:3306/science_direct?charset=utf8'
    ...
    def save_into_database_engine(self, table_to_save, table_name):
        engine = create_engine(self.sql_eng) 
        try:
            pd.io.sql.to_sql(table_to_save, table_name, engine, schema = 'science_direct', if_exists = 'append', index = False)
        except:
...
#        self.driver = driver
    ...
    def papers_in_an_issue(self, issue_url):
        ...
        if len(urls) == 0:
            urls = soup.find_all(class_='anchor article-content-title u-margin-xs-top u-margin-s-bottom')
        url_list = []
        for url in urls:
            url_list.append('https://www.sciencedirect.com' + url.get('href'))     
        if len(url_list) == 0:
            print('Warning.. cannot get information for %s' % issue_url )
        return url_list
...
#        for ids in range(531, len(journal_name['id'])):
            ...
            tmp_issue_list['iss_url'] = HtmlParser().all_issue_of_a_sd_journal(self.journal_name['id'][ids])
            tmp_issue_list['id'] = self.journal_name['id'][ids]
    ...
    def get_journal_paper_list(self):
        ...
        #for i, row in ret2.iterrows():    
            ...
            print('Paper list from journal #%s has been collected and stored...' % i)
...
def get_journal_paper_list_alter(issue_list, last_collected_id = '0924977X'):
    # issue_list has two fields, 'id' - journal id and 'issue_url' - issue's url
    sql = 'create table journal_paper_list2(index int auto_increment,\
            id varchar(100), vol varchar(100), paper_url varchar(300), PRIMARY KEY(id))' 
    DataSaver().database_execute_engine(sql)
    journal_paper_list = pd.DataFrame(columns = ['id', 'vol', 'paper_url'])
    if last_collected_id:
        start_index = issue_list[issue_list.id == last_collected_id].index.tolist()[-1] + 1
    for i in range(start_index, issue_list.shape[0]):
#        for i, row in self.journal_issue_list.iterrows():
        #for i, row in ret2.iterrows():    
        tmp_paper_list = {}
        tmp_paper_list['paper_url'] = HtmlParser().papers_in_an_issue(issue_list['iss_url'].iloc[i])
        tmp_paper_list['id'] = issue_list['id'].iloc[i]
        try: 
            tmp_paper_list['vol'] = issue_list['iss_url'].iloc[i].split('vol/')[1].split('/')[0]
        except:
            tmp_paper_list['vol'] = None 
        journal_paper_list = journal_paper_list.append(tmp_paper_list, ignore_index = True)
        print('Paper list from journal %s has been collected and up to %s paper urls has stored...' % (tmp_paper_list['id'], i))
        # store into database for every 100 issues
        if np.mod(i,100) == 0:
            DataSaver().save_into_database_engine(journal_paper_list, 'journal_paper_list2')
                #pd.io.sql.to_sql(journal_paper_list, 'journal_paper_list2', engine, schema = 'science_direct', if_exists = 'append')
            journal_paper_list = pd.DataFrame(columns = ['id', 'vol', 'paper_url'])
        return None
    
if __name__ == "__main__":

    issue_list = DataSaver().load_from_datebase_engine('select id,iss_url from journal_issue_list')
    last_collected_id = '0924977X'
    journal_paper_list = pd.DataFrame(columns = ['id', 'vol', 'paper_url'])
    if last_collected_id:
        start_index = issue_list[issue_list.id == last_collected_id].index.tolist()[-1] + 3
    else:
        start_index = 0    
    for i in range(start_index, issue_list.shape[0]): 
        tmp_paper_list = pd.DataFrame(columns = ['id','vol','paper_url'])
        ...
        try: 
            ...
            tmp_paper_list['vol'] = None 
        journal_paper_list = journal_paper_list.append(tmp_paper_list, ignore_index = True)
        print('Paper list from journal %s has been collected and up to #%s issue ...' % (issue_list['id'].iloc[i], i))

        if np.mod(i,20) == 0 or issue_list.shape[0] - i < 20:
            DataSaver().save_into_database_engine(journal_paper_list, 'journal_paper_list2')
            journal_paper_list = pd.DataFrame(columns = ['id', 'vol', 'paper_url'])
